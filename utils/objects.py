# Some utilities for objects handling
...
class Object:
    def __init__(self, _bounding_box, _type, _confidence, _tracker):
        """
        To init a new object that was detected and saves all the object parameters
        :param _bounding_box: the coordinates of the detected object
    ...
    def update(self, _bounding_box, _type=None, _confidence=None, _tracker=None):
        """
        To update the object with the new location, type, confidence and tracker
        :param _bounding_box: the new coordinates of the object
...
def add_new_objects(boxes, classes, confidences, objects, frame, tracker, mcdf, overlap_threshold):
    """
    This function creates new objects by the detected boxes or update existing one;
    it decides to update existing one if the overlap between them is greater than the overlap_threshold
    ...
    :param overlap_threshold: the maximum overlap between the detected object (boxes) to the tracked object
    (from objects) in order to update the object and not create new one
    :return: the objects
    """
    matched_object_ids = []
...
def remove_stray_objects(objects, matched_object_ids, mcdf):
    """
    To remove objects that "hang" around after a tracked object has left the frame.
...
def remove_duplicates(objects, overlap_threshold):
    """
    To remove duplicate objects, i.e objects that point to an already detected and tracked people
